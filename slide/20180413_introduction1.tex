...
\frame{
  ...
  \end{definition}
  後で出てくる直積を使うことで任意の関数を1引数関数と見なせるので、
...
\frame{
  ...
  （条件）
  任意の圏$\category{C}$の対象$W$と射$f: W \rightarrow X$, $g: W \rightarrow Y$に対して、
...
\frame{
  ...
  関数\code{h: Callable[[C], Tuple[A, B]]}があって次を満たす。
  \begin{align*}
    \code{h(x)[0] == f(x)} \\
    \code{h(x)[1] == g(x)}
  \end{align*}
  ...
  → \code{h(x) = (f(x), g(x))}と実装すれば条件を満たす。
}

\frame{
  \frametitle{関手 - 総称型}
  型の圏における関手は、ある条件を満たす総称型
  ``総称型''とは、任意の型を表す``型変数''を含んだ型のこと
  ...
  \item \code{List}を任意の型\code{A}を引数に取り型\code{List[A]}を返す関数と見なす。
    これがList関手の対象関数。
  \end{itemize}
}
...
\frame{
  \frametitle{Listの射関数}
  List関手の射関数は組み込み関数の\code{map}（Python 3では\code{list}）
...
\frame{
  ...
  \centerline{
    \begin{tabular}{l|l}
      ...
      \hline \hline
      命題 & 型 \\
      命題$P \subset Q$ & 型\code{Callable[[P], Q]} \\
      命題$P \land Q$ & Tuple型\code{Tuple[P, Q]}\\
      命題$P$の証明 & 型$P$を持つ式 \\
      命題$P$が証明可能 & 型$P$を持つ式が存在する \\
      \hline
    \end{tabular}
  }
}
